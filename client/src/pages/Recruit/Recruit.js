...
const Recruit = () => {
  ...
  let socketJs = new SockJS("https://localhost:8080/ws-stomp");
  ...
  useEffect(() => {
    const getContent = () => {
      ...
      fetch("https://localhost:8080/study/" + `${id}`, reqOption)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleDeleteRecruit = () => {
    ...
    fetch("https://localhost:8080/study/" + `${id}`, reqDelete)
      .then((res) => {
        if (res.ok) {
  ...
  const postCommentData = () => {
    ...
    fetch(`https://localhost:8080/study/${id}/comment`, reqPost)
      .then((res) => res.json())
      .then(() => {
  ...
  const handeDeleteComment = (elID) => {
    fetch(`https://localhost:8080/study/${id}/${elID}`, {
      method: "DELETE",
      headers: header,
    }).then(() => {
      setgetcondition(!getcondition);
  ...
  const handleEditRecruit = (id) => {
    fetch(`https://localhost:8080/study/${id}/comment`, {
      method: "DELETE",
    });
  ...
  const handleApplyStudy = (memberid, id) => {
    ...
    fetch(`https://localhost:8080/study/${id}/apply`, {
      method: "POST",
      headers: header,
    })
      .then((res) => {

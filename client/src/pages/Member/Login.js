...
// 3.35.188.110:8080
const KAKAOPATH =
  "https://localhost:8080/oauth2/authorization/kakao?redirect_uri=https://localhost:3000/Redirect";
...
export default function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    //14.6.86.98:8080 지훈님
    fetch("https://localhost:8080/login", reqOAuthPost)
      .then((response) => {
        if (response.ok) {
      ...
      .then((tokens) => {
        if (!tokens[0] || !tokens[1]) {
          ...
          if (tokens[0] !== null && tokens[1] !== null) {
            ...
            fetch("https://localhost:8080/member/locate", reqOAuthPost)
              .then((res) => console.log(res.json()))
              .then((res) => {
                fetch("https://localhost:8080/member/me", {
                  headers: {
                    "Content-Type": "application/json",
                    Accept: "application/json",
                    withCredentials: true,

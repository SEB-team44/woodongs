...
const SingleBoard = () => {
  ...
  useEffect(() => {
    function getBoardList() {
      ...
      fetch(`https://localhost:8080/post/${id}`, reqOption)
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
          return data;
  ...
  const handleDeleteSB = () => {
    ...
    fetch(`https://localhost:8080/post/${id}`, reqDelete)
      .then((res) => {
        if (res.ok) {
  ...
  const postCommentDatas = () => {
    ...
    fetch(`https://localhost:8080/post/${id}/comment`, reqPost)
      .then((res) => res.json())
      .then(() => {
  ...
  const handleDeleteSBComment = (elID) => {
    fetch(`https://localhost:8080/post/${id}/${elID}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        withCredentials: true,

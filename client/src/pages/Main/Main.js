...
const Main = () => {
  ...
  useEffect(() => {
    function getCardList() {
      ...
      if (getlat) {
        fetch(
          `https://localhost:8080/study/around?size=10&cursorId=100`,
          reqOption
        )
          .then((res) => {
            console.log("res", res);
        ...
        // fetch("https://localhost:8080/study/recruit/dummy", {
        //   method : "POST",
        //   headers : header,
        //   body: JSON.stringify({
        ...
        fetch(`https://localhost:8080/study?size=10&cursorId=100`, reqOption)
          .then((res) => res.json())
          .then((data) => {
            console.log(data);
            return data;
  ...
  function getCardList() {
    ...
    let url;
    if (cursor) {
      url = `https://localhost:8080/study?size=5&cursorId=${cursor}`;
    } else {
      url = `https://localhost:8080/study?size=10`;
    }
    if (!isAvailable) {
      return;
    }
